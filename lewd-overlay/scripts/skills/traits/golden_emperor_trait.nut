this.golden_emperor_trait <- ::inherit("scripts/skills/traits/character_trait", {
	...
	function getTooltip() {
		...
		if ("Lewd" in ::getroottable()) {
			ret.push({ id = 26, type = "text", icon = "ui/perks/lewd_alluring_presence.png",
				text = "[color=#DDA0DD]Imperial Charisma[/color] (Lewd): "
					+ (tier  >= 1  ? "[color=" + ::Const.UI.Color.PositiveValue + "]Alluring Presence[/color]" : "Alluring Presence (Purge I)") + ", "
					+ (level >= 20 ? "[color=" + ::Const.UI.Color.PositiveValue + "]Conqueror[/color]"         : "Conqueror (lv 20)") + ", "
					+ (level >= 35 ? "[color=" + ::Const.UI.Color.PositiveValue + "]Transcendence[/color]"     : "Transcendence (lv 35)") + ", "
					+ (tier  >= 4  ? "[color=" + ::Const.UI.Color.PositiveValue + "]Soul Harvest[/color]"      : "Soul Harvest (Purge IV)")
			});
		}
	...
	function _reconcileLewdPerks(_actor) {
		if (!("Lewd" in ::getroottable())) return;
		...
		if (tier >= 1) {
			this._grantLewdPerk(_actor,
				"scripts/skills/perks/perk_lewd_alluring_presence",
				"perk.lewd_alluring_presence", "Alluring Presence");
		}
		if (level >= 20) {
			this._grantLewdPerk(_actor,
				"scripts/skills/perks/perk_lewd_conqueror",
				"perk.lewd_conqueror", "Conqueror");
		}
		if (level >= 35) {
			this._grantLewdPerk(_actor,
				"scripts/skills/perks/perk_lewd_transcendence",
				"perk.lewd_transcendence", "Transcendence");
		}
		if (tier >= 4) {
			this._grantLewdPerk(_actor,
				"scripts/skills/perks/perk_lewd_soul_harvest",
				"perk.lewd_soul_harvest", "Soul Harvest");
		}
	}

	function _grantLewdPerk(_actor, _path, _id, _displayName) {
		try {
			local skills = _actor.getSkills();
			if (skills.getSkillByID(_id) != null) return;
			skills.add(::new(_path));
			if (!_actor.isHiddenToPlayer()
				&& "Tactical" in ::getroottable()
				&& ::Tactical != null
				&& ::Tactical.isActive())
			{
				::Tactical.EventLog.log("[color=#DDA0DD]Imperial Charisma — " + ::Const.UI.getColorizedEntityName(_actor) + " awakens to " + _displayName + ".[/color]");
			}
		} catch (e) {
			::logWarning("[mod_golden_throne] Emperor Lewd perk grant failed (" + _id + "): " + e);
		}
	}
